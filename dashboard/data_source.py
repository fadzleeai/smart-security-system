...
# ── Theme-aware inline colors — REMOVED ───────────────────────────────────────
# A THEME_COLORS dict used to live here, driven by st.context.theme.type,
# for inline-styled HTML strings in page1/2/3_*.py that couldn't read
# app.py's CSS var(--xxx) custom properties directly. Removed after a
# screenshot showed labels still rendering unreadable in dark mode
# despite that fix being in place — root cause confirmed directly via
# Streamlit's own documentation: st.context.theme.type "may be
# incorrect... when the app is first loaded within a session, or when
# the user changes the theme in the settings menu" (the two moments
# that matter most here). Replaced with plain CSS classes
# (.label-secondary, .track-bg, .divider-border in app.py) that use
# var(--text-secondary-on-card) etc. directly — resolved by the browser
# at the same time as the real background color, with no separate,
# uncertain Python-side read involved at all.
