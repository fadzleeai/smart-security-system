...
@st.cache_data(ttl=3)
def get_reviewed_strangers() -> set:
    """
    Filenames already handled via the popup — checked before showing the
    popup again so a stranger you already tagged doesn't keep reappearing
    on every 3-second auto-refresh. Cached 3s to match the autorefresh
    interval the popup uses, rather than hitting the Pi on every script run.
    """
    resp = _safe_get("/stranger/reviewed")
    if resp is None:
        return set()
    try:
        return set(resp.json().get("reviewed", []))
    except Exception:
        return set()
...
def get_active_alert() -> dict:
    ...
    if raw_img:
        basename = os.path.basename(str(raw_img))
        reviewed = get_reviewed_strangers()
        if basename and basename not in reviewed:
            return {
                "type":     "stranger",
                "filename": basename,
                "img_url":  _pi_image_url(basename),
            }
...
def get_full_logs() -> pd.DataFrame:
    ...
    try:
        ...
        return empty# Pi writes: Pending / Low / Medium / High
# Dashboard shows: None / Warning / Suspicious
THREAT_MAP = {
    # CONFIRMED against face_recognition_engine.py's actual _risk_label()
    # and _build_result() — these are the real, exact strings produced,
